	  	...
	  	$txt = "function set_values($myval){\n";
			...
			for($i=0;$i<count($this->column);$i++){
				...
				if($data == "chkbx[]"){	
					...
					#echo "opener.document.getElementById('$data').innerHTML = $sel";
					echo "$('#$data',window.opener.document).html($sel);\n";
					#echo "opener.document.$this->form.$data.setAttribute('style','background:#E0E0E0;border:1px #999 solid;padding:2px');\n";	
					...
					if($data != 'wpnpwp' && $data != 'paynpwp'){
						#echo "opener.document.$this->form.$data.setAttribute('readonly','readonly');\n";	
						#echo "opener.document.$this->form.$data.setAttribute('style','background:#E0E0E0;border:1px #999 solid;padding:2px;');\n";	
