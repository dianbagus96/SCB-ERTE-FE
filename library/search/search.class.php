<?php
	class Search {
		...
		function set_param($task,$impid,$kantor,$groupid='',$deptid='',$tipe=''){
			$this->task = $task;
			$this->impid = $impid;
			$this->kantor = $kantor;
		...
		function define_script(){
	  	...
	  	$txt = "function set_values($myval){\n";
			...
			for($i=0;$i<count($this->column);$i++){
				...
				if($data == "chkbx[]"){	
				...
				}elseif($data == 'branchsspcp'){					
					$sel = "'<option value='+myvalue0+'>'+myvalue0+' - '+myvalue1+'</option>;'\n";
					echo "$('#$data',window.opener.document).html($sel);\n";
					echo "$('#$data',window.opener.document).css({'background-color' : '#E0E0E0', 'border' : '1px #999 solid','padding':'2px'});\n";
					...
					echo "}else{\n";
						echo "$('#grouping0',window.opener.document).val(0).attr('checked','checked').removeAttr('disabled');\n";
						echo "$('#grouping1',window.opener.document).removeAttr('checked').attr('disabled','disabled');\n";
					echo "}\n";				
				}elseif($data == 'tipeacc'){
					$option1 = "<option value=\'1\' selected> - Saving Account</option>";
					$option2 = "<option value=\'2\' selected> - Current Account</option>";
					echo "if(myvalue$i==1){\n";
							echo "$('select#tipeacc',window.opener.document).html('$option1');\n";					
					echo "}else{\n";
							echo "$('select#tipeacc',window.opener.document).html('$option2');\n";					
					echo "}\n";	
					
				}else{					
					echo "$('#$data',window.opener.document).val(myvalue$i);\n";
					echo "$('#$data',window.opener.document).css({'border' : '1px #9BA8B5 solid','color' : '#000'});\n";
					if(!in_array($data,array('npwp','paynpwp','nama','address','city','zipcode','paynama','payaccount','groupid','deptid','groupname'))){
		...
		function define_search(){
			...
			if ($search_raw != ""){
				if ($search_cbx){
					$tambahan = " where upper($search_col) = upper('".$this->strFilter($search_raw)."') ";	
				}
				else{
					#$tambahan = " where upper($search_col) like upper('%".$this->strFilter($search_raw)."%') "; #asli
					$tambahan = " where ($search_col) like ('%".$this->strFilter($search_raw)."%') ";
				}
		...
		function drawForm(){
			...
			echo "			<input name=\"deptid\" type=\"hidden\" value=\"$deptid\">";
			echo "			<input name=\"tipe\" type=\"hidden\" value=\"$tipe\">";
  ...
  function sort_func(){
		...
		function drawTable(){
			...
			$table->cellspacing = 1;
			$table->frameborder = 0;
			$table->showPager(true,"BOTTOM",10,10);
