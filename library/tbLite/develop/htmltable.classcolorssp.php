...
class HTMLTableColorSSP{
	...
	*******************************************************************************/
	...
	function drawRow($count_column, $loop=''){
		if ($this->anyParent()){
			...
			if($this->warna=="ssp_terima"){
				if(trim($this->dataset->get(13))!="" && trim($this->dataset->get(13))!="SUCCESS"){
					echo '<tr style="border-bottom: 1px solid #D7D7D7; background:#FF7777;">';
				}elseif($loop % 2 == 0){
					echo '<tr style="border-bottom: 1px solid #D7D7D7; background:#E5EEF5;">';
				}else{
					echo '<tr>';			
				}
			}elseif($this->warna=="rekeningtax"){
				if(trim($this->dataset->get(5))=="Y"){
					echo '<tr style="border-bottom: 1px solid #D7D7D7; background:#FF7777;">';
				}elseif($loop % 2 == 0){
					echo '<tr style="border-bottom: 1px solid #D7D7D7; background:#E5EEF5;">';
				}else{
  ...
  function sort_func(){
		...
		echo "			};\n";
		echo "		});\n";	
		echo "	}else if(url == '".base_url."modul/rekeningtax/delete'){\n";	
		echo "		jConfirm('Apakah anda yakin untuk menghapus rekening ini?', 'Confirmation Dialog', function(r) {\n";
		echo "			if(r == true){\n";
		echo "				document.formtable.action = document.formtable.RDSelect.value;\n";
		echo "				document.formtable.method = \"post\";\n";
		echo "				document.formtable.submit();\n";
