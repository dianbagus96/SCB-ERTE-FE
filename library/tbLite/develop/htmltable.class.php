...
class HTMLTable{
	...
	*******************************************************************************/
	...
	function drawRow($count_column, $loop=''){
		if ($this->anyParent()){
			...
			if($this->showCBX){
				...
				}elseif($this->cbxMod2==true){
					echo "<td width=\"20\" nowrap><input type=\"checkbox\" name=\"cbx[".$loop."]\" flag_used='".$this->dataset->get(13)."' value=\"$cbxVal\" id='cbx".$loop."' ".$onclick."></td>";				
				}else{
	...
	$onclick .= " class='cbxBiasa'";
	if($this->cbxMod1==true){
		echo "<td style=\"border-bottom: 1px solid #D7D7D7;\" width=\"20\" align=\"center\" nowrap><input type=\"checkbox\" name=\"cbx[".$loop."]\" id='cbx".$loop."' value=\"$cbxVal\" ".$onclick." iddanamasuk='".$this->dataset->get(21)."' ></td>";
	}elseif($this->cbxMod2==true){
		echo "<td width=\"20\" nowrap><input type=\"checkbox\" name=\"cbx[".$loop."]\" flag_used='".$this->dataset->get(14)."' value=\"$cbxVal\" id='cbx".$loop."' ".$onclick."></td>";		
	}else{
				...
				if ($this->field[$i]->hidden){
					...
					if ($this->field[$i]->linker != ""){
						...
						if($this->ajaxMod1 == $i){
							...
							echo "<input type='text' id='ajaxMod18Text$loop' name='nilai_peb[]' style='text-align:right' disabled onkeyup=\"javascript:numberFormatKoma(this,',','','')\" value='$data'>";										
						}else{
							echo trim($data)==""?"&nbsp;":$data;
						}
					}
  ...
  function sort_func(){
		...
		if($this->ajaxMod1>-1){
			...
			if($this->ajaxMod18>-1){
				echo "		$('input#ajaxMod18Text'+no).attr('disabled','true');\n";		
			}
			echo "	}\n";
			echo "	totalNominal=0;\n";
			echo "	$('input.nominal').each(function(){\n";
		...
		echo "	}else if(url == '".base_url."ssp_print.php?cetak=bpn' || url == '".base_url."sspcp_print_new.php?pil=sspcp' ) {\n";
		...
		echo "		document.formtable.action = url;\n";
		echo "		document.formtable.method = \"post\";\n";
		echo "		document.formtable.target = \"pilihan\";\n";
		echo "		document.formtable.submit();\n";
		echo "		document.formtable.target = \"\";\n";				 
		...
		echo "	 }else if(url == '".base_url."modul/ssp/send' || url == '".base_url."modul/ssp/sendgroup' || url == '".base_url."modul/sspcp/send' ){\n";
		echo "	 	otp = prompt('Please input the Security Code :');\n";
		echo "		if($.trim(otp)!=''){\n";
		echo " 	     	$(a).attr('disabled','disabled');\n";		
		echo "			$('span#loadingproses').html('<img src=\"".base_url."img/loading.gif\" style=\"border:none\">').fadeIn(1000);\n";
		echo "			$.ajax({\n";
		echo "				type : 'POST',\n";
		echo "				data : 'otp='+otp,\n";
		echo "				url : '".base_url."security/validation',\n";
		echo "				success : function(msg){\n";
		echo "					$('span#loadingproses').html('');\n";
		echo "					hsl = jQuery.parseJSON(msg);\n";
		echo "					if(hsl.kode!='0000'){\n";
		echo "      					$(a).removeAttr('disabled');\n";
		echo "      					if(hsl.kode=='x'){\n";
		echo "							alert('Message : '+hsl.status);\n";
		echo "      						window.location.href='".base_url."modul/manage/activation';\n";	
		echo "      					}else{\n";		
		echo "							alert('Message : '+hsl.status);\n";
		echo "      					}\n";		
		echo " 						return false;\n";
		...
		echo "		document.formtable.method = \"post\";\n";
		echo "		document.formtable.submit();\n";				
		echo "	 }else if(url == '".base_url."modul/danamasuk/danaterlaporkanpilihpeb'){\n";#~
		echo "		if(valid>1){jAlert('Maaf proses tidak dapat dilakukan, silakan pilih salah satu!'); return false;}\n";		
		echo "     	$('input:checkbox.cbxBiasa').each(function(){ if($(this).attr('checked')==true){ id=$(this).val(); status=$(this).attr('flag_used')} });\n";
		echo "		if(status!='Pending'){jAlert('Pilih PEB hanya untuk Dana Masuk dengan status Pending!'); return false;}\n";		
		echo "		document.formtable.action = document.formtable.RDSelect.value+'/'+id+'/'+calcMD5(id);\n";
		echo "		document.formtable.method = \"post\";\n";
		echo "		document.formtable.submit();\n";	
		echo "	 }else if(url == '".base_url."modul/danamasuk/eksporrte' ){\n";	
		echo "		if($('#statusDanamasuk').val()==''){ jAlert('Status Dana Masuk Belum dipilih'); return false;}\n";
		...
		echo "		}\n";
		echo "		if(total.toFixed(2) - ".$this->nominalDiterima.">0){\n";
		...
		echo "			jAlert('Sandi Keterangan harus diisi!'); return false;\n";
		echo "		}\n";
		echo "		if(total.toFixed(2) > ".$this->nominalDiterima."){\n";
		echo "			jAlert('Nilai DHE tidak boleh melebihi Nominal yang diizinkan!<br> Total nilai DHE : '+money_format(''+total.toFixed(2)+'')+' , sementara <br>Dana yang diizinkan sebesar : ".number_format($this->nominalDiterima,0,'',',')."'); return false;\n";
		echo "		}\n";
		echo "		document.formtable.action = document.formtable.RDSelect.value;\n";
		echo "		document.formtable.method = \"post\";\n";
		...
		echo "			if($(this).attr('check')=='1'){\n";
		echo "				if($(this).val()==0 && $('input:checkbox#ajaxMod9Text'+no).attr('checked')==false){\n";	
		echo "					err++;\n";	
		...
		echo "			};\n";
		echo "		});\n";	
		echo "	}else if(url == '".base_url."modul/user/delete' || url == '".base_url."modul/rekening/delete' || url == '".base_url."modul/cabang/delete' || url == '".base_url."modul/kodemap/delete' || url == '".base_url."modul/email/delete'){\n";	
		...
		echo "			};\n";
		echo "		});\n";	
		echo "	}else if(url == '".base_url."modul/groupid/delete'){\n";	
		echo "		jConfirm('Apakah anda yakin untuk menghapus data Corp ini beserta user-usernya ?', 'Confirmation Dialog', function(r) {\n";
		echo "			if(r == true){\n";
		echo "				document.formtable.action = document.formtable.RDSelect.value;\n";
		echo "				document.formtable.method = \"post\";\n";
		echo "				document.formtable.submit();\n";
