				...
				if ($this->field[$i]->hidden){
					...
					if ($this->field[$i]->linker != ""){
						...
						if($this->ajaxMod1 == $i){
							...
							echo "<input type='text' id='ajaxMod1Text$loop' kurs='".($kurs)."' nilaipeb='".($nilaipeb)."' nilaitransfer='".($nilaipeb)."' class='nominal' name='nominal[]' style='text-align:right' disabled onkeyup=\"javascript:numberFormat(this,',','','')\" noid='".$loop."'>";
							...
							echo ($this->dataset->get(17)=='email')? 
  ...
  function sort_func(){
		...
		if($this->ajaxMod1>-1){
			...
			if($this->ajaxMod13==true){
				...
				echo "		nilaiPEB=$('input#ajaxMod17Text'+no).val();\n";
		...
		echo "		})\n";
		echo "		if(err>0){\n";
		echo "			jAlert('Hanya diperbolehkan untuk PEB yang berstatus Complete'); return false;\n";
		echo "		}\n";	
		echo "		document.formtable.action = document.formtable.RDSelect.value;\n";
		echo "		document.formtable.method = \"post\";\n";
		echo "		document.formtable.submit();\n";
