	  	...
	  	echo "</tr>";
			...
			for ($i = 0; $i <= $count_column-1; $i++) {
				if (!$this->field[$i]->hidden){
					...
					echo '<th width="'.$this->field[$i]->width.'" style="background:url('.base_url.'img/tab3.gif) repeat-x; height:22px; padding-left:7px; color:#ffffff; font-size:11px; font-family:arial; font-weight:bold; cursor: pointer;text-align:left" onclick="descasc(\''.$this->field[$i]->name.'\', \''.$_SERVER['REQUEST_URI'].'\');"><input type="hidden" value="'.$_GET['order'].'" id="descasc'.$this->field[$i]->name.'">';					
					}
					echo trim($this->field[$i]->headername);
					// draw sortable icon
					if (($this->field[$i]->sortable)&(strtoupper($this->field[$i]->name) == strtoupper($this->dataset->fieldName($i)))){
			...
			for ($i = 0; $i <= $count_column-1; $i++) {
				...
				if ($this->field[$i]->hidden){
					...
					if ($this->field[$i]->linker != ""){
						...
						if($this->ajaxMod1 == $i){
							$nilaipeb = (str_replace(',','',$this->dataset->get(17))>0)? str_replace(',','',$this->dataset->get(17)):0;
							$kurs = ($this->dataset->get(16)>0)? $this->dataset->get(16):0;
							echo "<input type='text' id='ajaxMod1Text$loop' kurs='".($kurs)."' nilaipeb='".($nilaipeb)."' nilaitransfer='".($nilaipeb)."' class='nominal' name='nominal[]' style='text-align:right' disabled onkeyup=\"javascript:numberFormat(this,',','','')\">";
						}elseif($this->ajaxMod2 == $i){
							echo "<input type='text' id='ajaxMod2Text$loop' name='sandiRTE[]' onclick=\"javascript:showSandiKeterangan('ajaxMod2Text".$loop."')\" style='cursor:pointer' disabled>";
						}elseif($this->ajaxMod3 == $i){
							echo "<input type='text' id='ajaxMod3Text$loop' val='".$data."' nominalDiterima='".str_replace(',','',$this->dataset->get(3))."' name='danaEkspor[]' value='".$data."' disabled style='text-align:right' class='danaEkspor' no='".$loop."' onkeyup=\"javascript:numberFormat(this,',','','')\">";
						}elseif($this->ajaxMod4 == $i){
							echo "<input type='text' id='ajaxMod4Text$loop' val='".$data."' name='danaNonEkspor[]' value='".$data."' disabled style='text-align:right' onkeyup=\"javascript:numberFormat(this,',','','')\" readonly>";	
							...
							echo (trim($data)=="")? "<input type='text' maxlength='6' style='font-size:11px;width:50px;' id='ajaxMod15Text$loop' name='nopeb[]' disabled onkeyup=\"javascript:numberFormat(this,'','','')\" val='".$data."' value='".$data."' class='nopeb'>":$data;
						}elseif($this->ajaxMod16 == $i){															
							echo (trim($this->dataset->get($this->ajaxMod15))=="")?'<input name="tglpeb[]" type="text" id="ajaxMod16Text'.$loop.'" value="'.$data.'" val="'.$data.'" size="10" autocomplete="off" readonly disabled onClick="if(self.gfPop)gfPop.fPopCalendar(this)" style="width:60px;" class="tglpeb">':$data;		
  ...
  function sort_func(){
		...
		if($this->ajaxMod1>-1){
			...
			if($this->ajaxMod13==true){
				...
				echo "					nilaipeb = $(this).attr('nilaipeb');\n";
				echo "					nilaipeb = eval(nilaipeb)/$(this).attr('kurs');\n";
				echo "					if(nilaipeb<=nilai){ $(this).val(money_format(nilaipeb.toFixed(2))); }\n";				
				echo "  			}\n";				
				echo "			}\n"; 
		...
		if($this->ajaxMod15>-1){
			...
			echo "		$('input#ajaxMod15Text'+no).val($('input#ajaxMod15Text'+no).atr('val'));\n";	
			echo "		$('input#ajaxMod16Text'+no).val($('input#ajaxMod16Text'+no).atr('val'));\n";	
			echo "	}\n";		
			echo "}\n";		
		}		
		...
		if(strstr($_SERVER['REQUEST_URI'],'FormSearchSSP.php') || strstr($_SERVER['REQUEST_URI'],'FormSearchSandiRTE.php') || strstr($_SERVER['REQUEST_URI'],'FormSearchReception.php')){
			...
			echo "		hal = (hal==0)?1:hal;
						...
						str = urls.split('/');
						url = (str.length>=6)? '".base_url."'+str[1]+'/'+str[2]+'/'+str[3]+'/'+str[4]+'/'+str[5]+'/'+hal : '".base_url."'+str[1]+'/'+str[2]+'/'+str[3]+'/'+hal;					
		...
		echo "	}\n";
		echo "}\n";
		echo "function descasc(name, urls){
				ada = 0;
				ada += (urls.indexOf('/asc')==-1)?0:1;
				ada += (urls.indexOf('/desc')==-1)?0:1;
				if(ada>0){
					str = urls.split('/');
					url = '';
					for(x=0;x<(str.length-2);x++){ url += str[x]+'/';}
				}else{
					url = urls+'/';
				}				
				if($('#descasc'+name).val()=='asc'){
					$('#descasc'+name).val('desc');
					document.location =url+name+'/desc';
				}else{
					$('#descasc'+name).val('asc');				
					document.location = url+name+'/asc';
				}
			  }";
