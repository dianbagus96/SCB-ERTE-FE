	  	...
	  	$txt = "function set_values($myval){\n";
			...
			for($i=0;$i<count($this->column);$i++){
				...
				if($data == "chkbx[]"){	
					...
					$option2 = "<option value=\'0\' selected> - Current Account</option>";
					echo "if(myvalue$i==1){\n";
		...
		function define_search(){
			...
			if ($search_raw != ""){
				if ($search_cbx){
					...
					#$tambahan = " where upper($search_col) like upper('%".$this->cleanstring($search_raw)."%') "; #asli
					$tambahan = " where ($search_col) like ('%".$this->cleanstring($search_raw)."%') ";
				}
